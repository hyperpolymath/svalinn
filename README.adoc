...
[.lead]
Svalinn is a Deno-based HTTP gateway that validates container operation requests and delegates to https://github.com/hyperpolymath/vordr[Vörðr]. It provides REST API access, JSON Schema validation, policy enforcement, OAuth2/JWT authentication, and a Docker Compose-compatible orchestration CLI (`svalinn-compose`).
...
|*svalinn-compose*
|Complete
|Docker Compose-compatible multi-container orchestration CLI
|===
...
Docker Compose-compatible orchestration with security extensions.
...
* *Licensing*: PMPL-1.0 OR AGPL-3.0-or-later, SPDX headers required
