...
Capabilities:
  strict:   drop ALL, add NONE
  balanced: drop ALL, add CHOWN,DAC_OVERRIDE,FOWNER,SETGID,SETUID

Security:
  read_only_rootfs:  strict=true   balanced=false
  (other settings identical)

Network:
  mode:              strict=none   balanced=bridge

Resources:
  pids_limit:        strict=100    balanced=500
  memory_limit:      strict=512M   balanced=2G
----
...
strict     Maximum security for production          ████████████ HIGH
balanced   Secure defaults with practical flex      ████████░░░░ MEDIUM
dev        Relaxed settings for development         ████░░░░░░░░ LOW

$ vordr profile show strict
Profile: strict
...
Seccomp: strict (custom profile)
Network: none (must explicitly enable)
...
[source,bash]
----
$ vordr profile set-default balanced
Default profile set to: balanced

$ vordr profile get-default
balanced
----
...
* `vordr/src/cli/profile.rs` (new) - Profile commands
* `vordr/src/profile/mod.rs` (new) - Profile module
* `vordr/src/profile/builtin.rs` (new) - Built-in profiles
* `vordr/src/profile/loader.rs` (new) - Profile loading
* `config/profiles/strict.toml` (new) - Strict profile definition
* `config/profiles/balanced.toml` (new) - Balanced profile
* `config/profiles/dev.toml` (new) - Dev profile

== Feature 14: Cross-Platform Machine

=== Overview

`vordr machine` provides a consistent container experience on macOS and Windows by managing a Linux VM that runs the actual container runtime.
...
----
$ vordr machine init
Initializing Vordr machine...

Platform: macOS (arm64)
Backend:  Apple Virtualization.framework

Downloading: vordr-vm-arm64.qcow2 (450MB)
[████████████████████████████████████████] 100%
...
Machine 'default' created successfully.

Run 'vordr machine start' to start the machine.
----

==== `vordr machine start/stop`

[source,bash]
----
$ vordr machine start
Starting machine 'default'...
Machine started. API available at /var/run/vordr.sock
...
----
$ vordr machine ssh
Welcome to Vordr Machine (Alpine Linux)
vordr@machine:~$ vordr ps
CONTAINER ID   IMAGE   STATUS
vordr@machine:~$
----

==== `vordr machine status`
...
State:   Running
CPUs:    4
Memory:  4.0 GB (2.1 GB used)
Disk:    50 GB (12.3 GB used)
API:     /var/run/vordr.sock
...
│  │  ┌─────────────────────────────────────────────────│─┐│  │
│  │  │  Vordr Machine VM (Alpine Linux)                │ ││  │
│  │  │                                                 │ ││  │
│  │  │  ┌───────────────────┐  ┌──────────────────┐   │ ││  │
│  │  │  │  vordr daemon     │◄─┤  gRPC server     │◄──┘ ││  │
│  │  │  │  (full engine)    │  │  (API bridge)    │     ││  │
│  │  │  └────────┬──────────┘  └──────────────────┘     ││  │
│  │  │           │                                       ││  │
│  │  │  ┌────────▼──────────┐                           ││  │
│  │  │  │  youki/runc       │                           ││  │
...
|Native Apple hypervisor, best performance

|macOS (x86_64)
|QEMU + HVF
|Hardware virtualization with HVF

|Windows
|WSL2 / Hyper-V
|Prefer WSL2 for simplicity
...
* `vordr/src/cli/machine.rs` (new) - Machine commands
* `vordr/src/machine/mod.rs` (new) - Machine module
* `vordr/src/machine/vm.rs` (new) - VM management
* `vordr/src/machine/api.rs` (new) - API bridge
* `vordr-machine/` (new) - Separate binary for daemon
...
=== Phase 7: Security Profiles (v0.4.0)

1. *Profile definitions* - strict/balanced/dev TOML files
2. *Profile CLI* - ls/show/diff/set-default commands
3. *Run integration* - --profile flag support
4. *Custom profiles* - User-defined profiles

=== Phase 8: Edge Shield + Explain (v0.4.0)

1. *Policy explainer* - Human-readable Gatekeeper errors
2. *Explain command* - `vordr explain` CLI
3. *Harden workflow* - `vordr harden` command
4. *Secure-run* - Guided container hardening

=== Phase 9: TUI Dashboard (v0.5.0)

1. *TUI framework* - ratatui setup
2. *Container view* - Live container list with stats
3. *Log streaming* - Real-time log display
4. *Quick actions* - Keyboard shortcuts for common tasks

=== Phase 10: Cross-Platform (v1.0.0)

1. *Machine CLI* - init/start/stop/ssh commands
2. *macOS backend* - Virtualization.framework
3. *Windows backend* - WSL2/Hyper-V
4. *API bridge* - gRPC over Unix socket

=== Success Criteria (COULD Features)

=== Edge Shield UX (COULD)

* [ ] `vordr explain` shows human-readable policy violations
* [ ] `vordr harden` analyzes container security
* [ ] `vordr secure-run` applies recommended hardening

=== TUI Dashboard (COULD)

* [ ] Live container list with status updates
* [ ] Log streaming with filter
* [ ] Quick exec into containers
* [ ] Keyboard shortcuts work

=== Security Profiles (COULD)

* [ ] `profile ls/show/diff` work
* [ ] `run --profile strict` applies all restrictions
* [ ] Custom profiles can be created
* [ ] Profile differences clearly shown

=== Cross-Platform (COULD)

* [ ] `machine init` creates VM on macOS
* [ ] `machine start/stop` manage lifecycle
* [ ] Containers run transparently through VM
* [ ] API bridge has acceptable latency (<10ms overhead)
