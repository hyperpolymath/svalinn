 ...
 * logic in Jwt.parseJwtSegments and Jwt.verifyJwt without requiring a real key.
 */
let makeToken = (
  ~alg: string="RS256",
  ~typ: string="JWT",
...
Deno.testSync("parseJwtSegments: rejects token with only one part", () => {
  Assert.assertThrows(() => {
    let _ = Jwt.parseJwtSegments("onlyonepart")
  })
})

Deno.testSync("parseJwtSegments: rejects token with two parts (missing signature)", () => {
  Assert.assertThrows(() => {
    let _ = Jwt.parseJwtSegments("header.payload")
  })
})

Deno.testSync("parseJwtSegments: rejects token with four parts (extra segment)", () => {
  // RFC 7519 §7.2 — compact serialisation MUST have exactly three segments.
  Assert.assertThrows(() => {
    let _ = Jwt.parseJwtSegments("a.b.c.d")
  })
})

Deno.testSync("parseJwtSegments: rejects empty string token", () => {
  Assert.assertThrows(() => {
    let _ = Jwt.parseJwtSegments("")
  })
})

Deno.testSync("parseJwtSegments: rejects token with empty header segment", () => {
  // An empty base64 segment cannot be parsed as JSON.
  Assert.assertThrows(() => {
    let _ = Jwt.parseJwtSegments(".payload.sig")
  })
})
...
Deno.testSync("parseJwtSegments: alg:none token is parsed structurally (header readable)", () => {
  // We must be able to READ the header to detect the attack — decoding must not
  // crash. The security check happens in verifyJwt, not parseJwtSegments.
  ...
  let header: {"alg": string} = Obj.magic(decoded.header)
  Assert.assertEquals(header["alg"], "none")
})
...
Deno.testSync("parseJwtSegments: token with missing sub claim is parseable (claim check is verifyJwt)", () => {
  ...
  let payload: {"sub": option<string>} = Obj.magic(decoded.payload)
  Assert.assertEquals(payload["sub"], None)
})

Deno.testSync("parseJwtSegments: token with missing exp claim is parseable", () => {
  ...
  let payload: {"exp": option<float>} = Obj.magic(decoded.payload)
  Assert.assertEquals(payload["exp"], None)
})
...
Deno.testSync("parseJwtSegments: rejects SQL-like injection in token string", () => {
  Assert.assertThrows(() => {
    let _ = Jwt.parseJwtSegments("' OR '1'='1")
  })
})

Deno.testSync("parseJwtSegments: rejects null-byte injection token", () => {
  Assert.assertThrows(() => {
    let _ = Jwt.parseJwtSegments("head\x00er.payload.sig")
  })
})

Deno.testSync("parseJwtSegments: rejects very large token (DoS guard)", () => {
  ...
  try {
    let _ = Jwt.parseJwtSegments(bigToken)
